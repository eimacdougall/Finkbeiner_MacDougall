...
mlflow.set_experiment("fashion_mnist_nn_project")
...
#Plot 5
importances = evaluate.ablation_importance_patch(model_classification, test_images, test_labels, patch_size=7, metric='accuracy')
evaluate.plot_patch_importance(importances, patch_size=7, save_path="models/patch_ablation_importance.png")

importance = evaluate.ablation_importance(model_classification, test_images, test_labels, metric='accuracy', save_path="models/feature_ablation_importance.png")

plt.figure(figsize=(10,5))
plt.bar(range(len(importance)), importance)
plt.xlabel("Feature Index")
plt.ylabel("Ablation Importance (Δ Accuracy)")
...
for i in input_slices:
    ...
    model = build_model()
    model.fit(x_train_ablated, train_labels, epochs=5, validation_split=0.15, verbose=0)
